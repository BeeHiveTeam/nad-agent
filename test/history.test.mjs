...
describe("resolveDeadline", () => {
  ...
  const cases = [
    ...
    // Not CEILING: 24.9 days is an unbounded wait wearing a deadline's clothes, which is the
    // state the deadline was added to remove. A finite request is a duration and gets capped;
    // Infinity names no duration and takes the default.
    ["Infinity", Number.POSITIVE_INFINITY, FALLBACK],
...
describe("getHistory — request deadlines", () => {
  it("returns the healthy endpoint's history when the other never sends headers", async () => {
    ...
    try {
      ...
      assert.equal(fixture.stalledSockets.length, 1, "the stalled request must have reached the fixture");
  ...
  it("returns the healthy endpoint's history when the other stalls mid-body", async () => {
    ...
    try {
      ...
      // properties of undefined", which points away from the cause on the one platform
      // where these tests have actually been unstable.
      assert.equal(fixture.stalledSockets.length, 1, "the stalled request must have reached the fixture");
  ...
  it("reports a request cancelled before the check as cancelled, without waiting", async () => {
    ...
    try {
      ...
      // answer that is already known settles on the microtask queue and beats a macrotask
      // every time, while one that subscribes to a close already past loses to it every
      // time. There is no wall clock here to be flaky about on a loaded runner.
  ...
  it("ignores a deadline that is not a usable number instead of timing out at once", async () => {
    ...
    try {
      // What these rows prove is narrow on purpose: that this path routes through
      // resolveDeadline rather than handing the raw value to setTimeout. Passed through, every
      // one of them fires at about a millisecond and cancels the request. Which non-cancelling
      // value each produces — the default or the cap — is not observable here, because a
      // fixture that answers at once cannot tell ten seconds from twenty-four days; that is
      // pinned in the resolveDeadline table instead.
      for (const timeoutMs of [Number.NaN, 0, -1, 0.5, Number.POSITIVE_INFINITY, "soon", null]) {
        ...
        assert.equal(result.length, 1, `timeoutMs=${String(timeoutMs)} must not cancel the request`);
      }
    } finally {
      fixture.close();
    }
